...
export default function Home() {
  ...
  return (
    <div className="min-h-screen w-full max-w-full overflow-x-hidden bg-neutral-950 text-white p-4 sm:p-6 md:p-12 font-sans py-12 sm:py-24">
      ...
      <div className="max-w-4xl mx-auto space-y-10 w-full min-w-0">
        ...
        <div className="grid md:grid-cols-2 gap-6 sm:gap-8 w-full min-w-0">
          {/* قسم الإنشاء */}
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-neutral-900 border border-neutral-800 p-5 sm:p-6 rounded-2xl space-y-5 w-full min-w-0 overflow-hidden">
            ...
            <form onSubmit={handleCreateQR} className="space-y-4 w-full min-w-0">
              <div className="w-full min-w-0">
                <label className="block text-xs text-neutral-400 mb-2">الرابط الموجه إليه (Destination URL):</label>
                <div className="relative w-full min-w-0">
                  <Link2 className="absolute right-3 top-3.5 text-neutral-500 pointer-events-none" size={18} />
                  <input
                    type="url"
                    required
                    value={targetUrl}
          ...
          {/* قسم التحديث */}
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-neutral-900 border border-neutral-800 p-5 sm:p-6 rounded-2xl space-y-5 w-full min-w-0 overflow-hidden">
            ...
            <div className="space-y-4 w-full min-w-0">
              ...
              <label className="border-2 border-dashed border-neutral-800 hover:border-cyan-500/50 bg-neutral-950 rounded-lg p-6 flex flex-col items-center justify-center gap-2 cursor-pointer transition w-full">
              ...
              {extractedId && (
                <form onSubmit={handleUpdateQR} className="space-y-4 pt-4 border-t border-neutral-800 w-full min-w-0">
                  {/* صندوق تفاصيل الـ QR المقروء والرابط الحالي */}
                  <div className="bg-neutral-950 border border-neutral-800 rounded-xl p-3.5 sm:p-4 space-y-3 text-xs w-full min-w-0 overflow-hidden">
                    <div className="flex items-center justify-between gap-2 text-emerald-400 w-full min-w-0">
                      <span className="flex items-center gap-1.5 font-medium shrink-0">
                        <CheckCircle2 size={15} /> المعرف (ID):
                      </span>
                      <span className="font-mono text-white bg-neutral-900 px-2 py-0.5 rounded border border-neutral-800 truncate min-w-0 dir-ltr text-right">
                        {extractedId}
                      </span>
                    </div>

                    <div className="flex flex-col gap-1.5 border-t border-neutral-900 pt-2.5 w-full min-w-0">
                      <span className="text-neutral-400">الرابط الحالي المسجل:</span>
                      {fetchingCurrentUrl ? (
                        <span className="flex items-center gap-2 text-indigo-400 py-1">
                          <Loader2 size={14} className="animate-spin shrink-0" /> جاري جلب الرابط الحالي من الشيت...
                        </span>
                      ) : (
                        <a
                          href={currentUrl}
                          target="_blank"
                          rel="noreferrer"
                          className="text-cyan-400 hover:text-cyan-300 font-mono text-xs flex items-center gap-2 hover:underline bg-cyan-950/30 p-2.5 rounded border border-cyan-900/40 w-full min-w-0 overflow-hidden dir-ltr"
                        >
                          <ExternalLink size={14} className="shrink-0" />
                          <span className="truncate min-w-0 flex-1">{currentUrl || "لا يوجد رابط"}</span>
                        </a>
                      )}
                    </div>
                  </div>
